...
class YumUserController extends YumController {
	...
	public function accessRules() {
		return array(
				...
				array('allow',
					'actions'=>array('admin','delete','create','update', 'list', 'assign','import', 'generateData', 'csv'),
	...
	public function actionCreate() {
		...
		$this->render('create',array('model' => $model,'passwordform' => $passwordform,'profile' => isset($profile) ? $profile : null,));
	}
	...
	public function actionImport()
	{
		...
		{
		    $degree->attributes = $_POST['Degree'];
			$degreeID = $degree->degreeName;
	    	$model->attributes=$_POST['Import'];
			$file=CUploadedFile::getInstance($model,'list');
			if($model->validate())
			{
            ...
            for ($row = 2; $row <= $highestRow; ++$row) 
			{
			    ...
			    $user->salt = YumEncrypt::generateSalt();
				$user->password = YumEncrypt::encrypt('coict2013', $user->salt);
				...
				$firstname  =   $objWorksheet->getCell('A'.$row)->getValue();
				$middlename =   $objWorksheet->getCell('B'.$row)->getValue();
				$lastname   =   $objWorksheet->getCell('C'.$row)->getValue();
				$regno 	    =   $objWorksheet->getCell('D'.$row)->getValue();
				$email	    =   $objWorksheet->getCell('E'.$row)->getValue();
				$phone	    =   $objWorksheet->getCell('F'.$row)->getValue();
				$year_of_study = $objWorksheet->getCell('G'.$row)->getValue();
         
				$sql = "INSERT INTO user (username, password,salt,activationKey,createtime,lastpasswordchange,status,notifyType,student) 
				VALUES (:username, :password, :salt, :activationKey, :createtime,:lastpasswordchange, :status, :notifyType, :student)";
				$command = Yii::app()->db->createCommand($sql);
				$command->bindValue(":username", trim($regno), PDO::PARAM_STR);
				$command->bindValue(":password",$user->password , PDO::PARAM_STR);
				$command->bindValue(":salt", $user->salt, PDO::PARAM_STR);
				$command->bindValue(":activationKey", $user->activationKey, PDO::PARAM_STR);
				$command->bindValue(":createtime", $user->lastpasswordchange, PDO::PARAM_INT);
				$command->bindValue(":lastpasswordchange",$user->lastpasswordchange, PDO::PARAM_INT);
				$command->bindValue(":status", 1, PDO::PARAM_INT);
				$command->bindValue(":student", 1, PDO::PARAM_INT);
				$command->bindValue(":notifyType", 'Instant', PDO::PARAM_STR);
				...
				if ($upload)
				{
					...
					while($dataReader->read()!==false)
					{
					 ...
					 $sql = "INSERT INTO profile (user_id, lastname,firstname,email,regno,middle_name,phone,degreeID,year_of_study) 
						VALUES (:user_id, :lastname, :firstname, :email, :regno,:middlename, :phone, :degreeID, :year_of_study)";
						$command = Yii::app()->db->createCommand($sql);
						$command->bindValue(":user_id", $user_id, PDO::PARAM_INT);
						$command->bindValue(":lastname",trim($lastname) , PDO::PARAM_STR);
						$command->bindValue(":firstname", trim($firstname), PDO::PARAM_STR);
						$command->bindValue(":email", trim($email), PDO::PARAM_STR);
						$command->bindValue(":regno", trim($regno), PDO::PARAM_STR);
						$command->bindValue(":middlename",trim($middlename), PDO::PARAM_STR);
						$command->bindValue(":phone", trim($phone), PDO::PARAM_STR);
						$command->bindValue(":degreeID", $degreeID, PDO::PARAM_INT);
						$command->bindValue(":year_of_study", trim($year_of_study), PDO::PARAM_INT);
					    $uploaded = $command->execute();
